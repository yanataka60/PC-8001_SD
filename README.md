...
　J5 2x25Pinコネクタは、拡張端子と同じ配列です。フラットケーブルを接続してRAMボード等を接続できますが、接続する向きを間違えないようにしてください。
...
|エントリポイント|名前|用途|
| -------------- | -- | -- |
|7000|EXIST|SD FILE EXIST CHECK|

　HL <- DOS_FILENAME先頭アドレス

　HLレジスタが示すDOS_FILENAMEのファイルがSDに存在するかチェックする、またSDカードの抜き差し状態のチェックも兼ねている

　DOS_FILENAMEは32文字まで、最後に00Hをつけること。

　Aレジスタ以外のレジスタは変化しない。

　SD正常 ファイル存在する   Aレジスタ=00h

　SD正常 ファイル存在しない Aレジスタ=F1h

　SD異常                    Aレジスタ=F0h

|エントリポイント|名前|用途|
| -------------- | -- | -- |
|7FFB|VERSION|VERSION FLAG|

　SD用拡張ROMのバージョンを示します。

　04h:2024.8.6バージョン
　以外:2024.8.6バージョンより以前のバージョン又は未知のバージョン
